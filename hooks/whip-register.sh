...
TR=$(printf '%s' "$PAY" | jq -r '.transcript_path // empty')
mkdir -p "$REG" "$DIR/s" "$DIR/b"
F="$REG/$SID.json"
NAME=$(jq -r '.name // empty' "$F" 2>/dev/null)
NOW=$(date +%s)
jq -n --arg id "$SID" --arg cwd "$CWD" --arg tr "$TR" --arg name "$NAME" \
      --argjson pid "$PPID" --argjson now "$NOW" \
  '{id:$id, cwd:$cwd, name:$name, pid:$pid, started:$now, seen:$now, busy:false, transcript:$tr}' > "$F"
exit 0
